...
const DigitalPetalsShader = () => {
  ...
  useEffect(() => {
    ...
    // 2) GLSL Shaders (Los pétalos matemáticos)
    const vertexShader = `
      void main() {
    ...
    const fragmentShader = `
      ...
      void main() {
        ...
        // Colores adaptados a tu tienda "PARA TI" (Rojos, rosas y morados oscuros)
        vec3 color1         = vec3(0.6, 0.1, 0.3); 
        vec3 color2         = vec3(0.9, 0.3, 0.4); 
        vec3 highlightColor = vec3(1.0, 0.8, 0.8);
  ...
  return (
    <div 
      ...
      aria-label="Fondo interactivo de pétalos digitales"
    />
  );
};
